...
void Application::InitVariables(void)
{
	...
	for (uint i = 0; i < 1; i++) {
		...
		*pMatrix = glm::translate(IDENTITY_M4, vector3(i*2.0f, 0.0f, 0.0f));
		m4List.push_back(pMatrix);
		//m4List.push_back(glm::translate(IDENTITY_M4, vector3(i*2.0f, 0.0f, 0.0f)) * ToMatrix4(m_qArcBall));
	}
...
void Application::Display(void)
{
	...
	/*for (uint i = 0; i < 5000; ++i) {
		*m4List[i] = glm::translate(IDENTITY_M4, vector3(i * 2.0f, 0, 0)) * ToMatrix4(m_qArcBall);
	}
	*/
	...
	*m4List[0] = ToMatrix4(m_qArcBall);
	...
	m_pRB->Render(m_pCamera, *m4List[0]);
...
{
	//Delete the entries in the pointer array
